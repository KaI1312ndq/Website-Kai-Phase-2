...
export async function GET(req: NextRequest) {
  ...
  const remainingDrafts = DRAFT_POSTS.filter(
    (p) => !BATCH_2_3_IDS.has(p.id) && !BATCH_4_IDS.has(p.id),
  );
  ...
  for (const post of allPosts) {
    try {
      ...
      const enrichedMd = injectAutoImages({
        postId: post.id,
        category: meta?.category ?? post.category ?? "default",
        markdown: post.content,
      });
      const body = mdToBlocks(post.id.replace(/[^a-zA-Z0-9]/g, "_") + "_", enrichedMd);
      const doc = {
        _id: post.id,
        _type: "post",
        title: post.title,
